...
[README.md](README.md) describes the project: how the stages fit together, the annotator, the export and the results so far. The stages are named 1A–1I in code and docs. [pipeline/README.md](pipeline/README.md) documents every stage in detail: algorithms, measured numbers, outputs and known limits. Read the relevant section before changing a stage.

**Status (2026-09-12):** 1A (video prep), 1B (court homography) and 1C (TrackNetV3 shuttle tracking) are done. 1D (contact detection, `contact_detect.py`) is built but short of its gate on the ShuttleSet test matches (recall 79-84%, precision 83-84% at ±2 frames); the user chose to move on. 1E (players, `player_detect.py`) passed its gate (right hitter at 98.1%). 1F (`feature_assemble.py`) wrote `data/events.json`: 5,999 events in 433 rallies from the five 2025 matches, validated; the other half of its gate (the user checking the court diagrams in the annotator) is pending. 1G is built as `shot_classify.py`, a classifier trained on ShuttleSet rather than Claude vision labelling: on the pipeline's events for the ShuttleSet test matches it's right at 70-74% (top-3 92%), short of the 75% gate, and its suggestions are in every event's `model_label`. Claude vision labelling isn't built (`claude_label: null`); it's the fallback for the types the numbers can't separate. 1H (annotating) is under way. 1I's first run (`annotated_experiment.py` in the predictor repo, trained on 24 labelled rally pieces alone) scored 44.5% on the predictor's validation split, against 38.9% for as many ShuttleSet rallies. A next-shot table from the labels gets 52.1%, against 55.9% from all of ShuttleSet's training data. Data size is the limit.
...
- **Events and storage:** events come from `/data/events.json` (written by `feature_assemble.py`; the dev server serves the repo root) and fall back to `generateMockEvents` when it's missing. Only the review persists, as `{event id: { annotation, gap_before, landing_fix }}` (`landing_fix` is a landing moved on the court with L, normalised; the export prefers it), through `window.storage.get/set`, which Claude artifacts provide and `main.jsx` shims onto `localStorage`. `STORAGE_KEY` is `annotator:review:v4` (v3's bare annotations are read once as a fallback). An annotation is either a label `{ shot_type, hitting_player, confirmed, corrected, timestamp }` or `{ not_shot: true, timestamp }`. Bump the version when the review shape or label set changes incompatibly. `POSITION_KEY` separately remembers the event on screen, the filter and the match picked in the match picker (one match at a time, or all), so a reload continues there (or at the first unreviewed event if that one is gone). When another tab saves the review (a `storage` event), a tab stops saving and asks for a reload. Before that guard, a tab left open from an earlier session re-sent its stale export on a hot reload and overwrote `annotations.csv`.
- **Disk save:** a plugin in `vite.config.js` handles `PUT /api/annotations.csv` and overwrites `./annotations.csv` (or `$ANNOTATIONS_CSV`), and `PUT /api/review.json`, the whole review, which overwrites `./review.json` (or `$REVIEW_JSON`). Before a save that would shrink the review by more than 5 events, the old file is copied to `review.<time>.json`. The app sends both on every change, queued so writes land in order, and never sends an empty review except from Reset (which asks first and clears the disk copy too). A browser without a stored review loads `/review.json`. This only works under `npm run dev` or `preview`; otherwise the Download button is the fallback. The dev watcher ignores `*.csv`, `pipeline/` and `data/`.
...
- `FrameStrip` shows the middle three frames (f-1, contact, f+1) of the event's strip image (1F writes f-2..f+2, cropped on the hitter and shuttle), or placeholders for mock events. The layout is the 3D court on the left and, on the right, the strips for this shot and the next one in the rally (its contact is where this shot went), then the label and controls. The strips stay on fixed rows per player: `players[0]` on top, `players[1]` below. This shot sits on its hitter's row, and the next shot (or a last shot's `frames_after`) on the other row. The suggestion sits between the two strips, so the frames and the label read in one glance.
...
- A shot whose landing is on its hitter's own half (`onOwnHalf`) is left out and the rally split there, until the review fixes it with L or X, and the annotator flags it. That includes a rally's last shot: the user's call on 2026-09-15, although most such last shots in ShuttleSet are net errors.
...
- **Landings (the user's rule).** A landing on its hitter's own half is never accepted as real, a rally's last shot included: either the landing is wrong or a hit is false. Flag or drop it, count it, and never draw conclusions from it. In ShuttleSet, 687 of the 708 last shots landing on their own side are net errors; on 2026-09-15 the user chose to reject those too.
